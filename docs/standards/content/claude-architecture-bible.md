...
- **Agent definitions**: Agents are loaded from marketplace or defined locally
- **Project configuration**: `.claude-src/config.yaml` (simplified skill assignments)
...
| **Skills** | `.claude/skills/`                | Define _implementation patterns_    | Loaded from marketplace |
| **Config** | `.claude-src/config.yaml`        | Control which agents/skills compile | YES - project-specific  |

**Example:**

- `web-developer` agent (generic): "You implement React components following patterns"
- `web-framework-react` skill: "Use Zustand, SCSS Modules, React Query"
- Same agent + different skill: "Use Redux, Tailwind, custom patterns"
...
3. Agents defined in config are compiled
4. Each agent gets bundled with assigned skills
5. Skills are loaded from marketplace or local overrides

**Result:** Configured agents and skills are compiled to `.claude/` for Claude Code.

## Directory Structure

```
src/
├── agents/                    # Agent source files (GENERIC - shared across projects)
...
│   └── {agent-name}.md        # Clean names (e.g., web-developer.md)
└── skills/                    # Compiled skills (flat structure)
    └── {skill-id}/            # e.g., web-framework-react/
        ├── SKILL.md           # Main skill content
        └── metadata.yaml      # Skill metadata
```

> **NOTE:** The `src/stacks/` directory no longer exists. Stacks are now semantic groupings defined via the `stack:` field in `.claude-src/config.yaml`.
...
In `.claude-src/config.yaml`, skills use a unified array structure:
...
| --------------------- | ------------------------- | ----------------------------------------------------- |
| web-developer         | web-reviewer              | Files changed, patterns followed, a11y/perf checks    |
| api-developer         | api-reviewer              | API docs, security checks, DB changes, error handling |
| web-reviewer          | developer                 | Issues with severity, code fixes, checklist results   |
| api-reviewer          | developer                 | Security audit, issues with fixes, convention check   |
| web-researcher        | web-developer             | Component props, state patterns, styling tokens       |
| api-researcher        | api-developer             | Route handlers, DB schemas, middleware chains         |
| web-architecture      | developer/user            | Scaffolding complete, pattern refs, setup guide       |
| web-pm                | developer/tester          | Spec with patterns, success criteria, constraints     |
| web-tester/cli-tester | developer                 | Failing tests, expected behaviors, mocking setup      |
| pattern-scout         | web-pattern-critique/docs | Exhaustive catalog, not recommendations               |
| web-pattern-critique  | developer/user            | Issues by severity, refactoring strategies            |
...
- Agent definitions live alongside their source files
- Skills are assigned via `.claude-src/config.yaml`, not in agent definitions
- Core principles are embedded directly in the template (not configurable per agent)

## Config.yaml Structure (Simplified)

Project configuration lives at `.claude-src/config.yaml` with a simplified format:

1. **Skills list** - flat array of skill IDs to install
2. **Agent configurations** - optional skill assignments per agent
3. **Stack grouping** - semantic `stack:` field for organizing skills

**Unified Skills**: All skills are loaded via the Skill tool when the agent evaluates them as relevant. There is no distinction between precompiled and dynamic skills.
...
# Optional: Agent-specific skill assignments
agents:
  web-developer:
    skills:
      - id: web-framework-react
...
Agents are **generic** - define them once in their source directory, then enable them in projects.
...
├── output-format.md          # OPTIONAL: Response structure
...
### Step 3: Assign Skills in Project Config

Add skill assignments to `.claude-src/config.yaml`:
...
Skills are loaded from the marketplace or created locally. They follow a flat directory structure.
...
```markdown
---
name: web-framework-react (@author)
...
See "Skill Schema Requirements" section below for full schema.

### Key Points

- Skills are loaded from marketplace or local `.claude/skills/` directory
- All skills are loaded via the Skill tool (no precompiled embedding)
- Skill source = compiled (flat structure in `.claude/skills/`)
...
| `author`          | string        | Author handle with @ prefix       | `@vince`, `@community`                  |
