...
**Path:** `docs/bibles/CLAUDE_ARCHITECTURE_BIBLE.md`
...
- Directory structure (`src/agents/{category}/{agent}/`, `.claude/skills/`)
- Agent categories (developer, reviewer, researcher, planning, pattern, meta, tester, migration)
- How agents are generic (role + workflow) while skills are stack-specific (implementation patterns)
- Adding new agents and skills
- Template system (`agent.liquid` bundled in CLI)
- **Skill schema requirements** (metadata.yaml, SKILL.md frontmatter, validation rules)

**Use when:** Creating or modifying agents, understanding the build system, authoring new skills.
...
**Path:** `docs/bibles/PROMPT_BIBLE.md`
...
**Path:** `docs/bibles/DOCUMENTATION_BIBLE.md`
...
**Path:** `docs/bibles/SKILL-ATOMICITY-BIBLE.md`
...
**Path:** `docs/plugins/PLUGIN-DEVELOPMENT.md` _(TODO: create this file)_
...
Complete reference for CLI commands. See [commands.md](../commands.md).
...
| Content                             | Repository         |
| ----------------------------------- | ------------------ |
| Skills (`.claude/skills/`)          | Skills Repo (this) |
...
| `commands.md`               | `docs/`                         | CLI command reference                                      |
...
5. **Skills are source of truth**: `.claude/skills/` has full metadata, agents are compiled output
...
Additional methodology guidance (context management, improvement protocols, etc.) is available via **methodology skills** in `.claude/skills/meta-methodology-*/`.
...
| `migration/`  | `src/agents/migration/`  | Migration agents                  |

### Development Agents

| Agent              | Path                               | Role                                    |
| ------------------ | ---------------------------------- | --------------------------------------- |
| **web-developer**  | `.claude/agents/web-developer.md`  | Implements frontend features from specs |
| **api-developer**  | `.claude/agents/api-developer.md`  | Implements backend features from specs  |
| **cli-developer**  | `.claude/agents/cli-developer.md`  | Implements CLI features from specs      |
| **web-researcher** | `.claude/agents/web-researcher.md` | Read-only frontend pattern discovery    |
| **api-researcher** | `.claude/agents/api-researcher.md` | Read-only backend pattern discovery     |
| **web-tester**     | `.claude/agents/web-tester.md`     | Writes web tests BEFORE implementation  |
| **cli-tester**     | `.claude/agents/cli-tester.md`     | Writes CLI tests BEFORE implementation  |

### Review Agents

| Agent            | Path                             | Role                                                 |
| ---------------- | -------------------------------- | ---------------------------------------------------- |
| **web-reviewer** | `.claude/agents/web-reviewer.md` | Reviews React components (`.tsx`/`.jsx`)             |
| **api-reviewer** | `.claude/agents/api-reviewer.md` | Reviews non-React code (API routes, configs, CI/CD)  |
| **cli-reviewer** | `.claude/agents/cli-reviewer.md` | Reviews CLI code (Commander.js, prompts, exit codes) |

### Architecture & Planning Agents

| Agent                | Path                                 | Role                                                      |
| -------------------- | ------------------------------------ | --------------------------------------------------------- |
| **web-architecture** | `.claude/agents/web-architecture.md` | Scaffolds new applications with all foundational patterns |
| **web-pm**           | `.claude/agents/web-pm.md`           | Creates detailed implementation specs                     |
...
| **web-pattern-critique** | `.claude/agents/web-pattern-critique.md` | Critiques patterns against industry standards     |

### Migration Agents

| Agent            | Path                             | Role                                     |
| ---------------- | -------------------------------- | ---------------------------------------- |
| **cli-migrator** | `.claude/agents/cli-migrator.md` | Converts Commander.js CLI to oclif + Ink |
...
| `CLI-FRAMEWORK-RESEARCH.md`       | _(TODO: create this file)_      | Framework comparison (@clack vs Ink vs Inquirer) |
...
| Document          | Path                            | Purpose            |
| ----------------- | ------------------------------- | ------------------ |
| `TODO.md`         | Root                            | Current TODO items |
...
| Document            | Path           | Purpose                                                              |
| ------------------- | -------------- | -------------------------------------------------------------------- |
| `FRONTEND_BIBLE.md` | `docs/bibles/` | Frontend patterns and conventions (decision trees, styling, testing) |
...
| `config.yaml` | `.claude-src/` | Project skill/agent configuration                        |
