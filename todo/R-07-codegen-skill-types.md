...
Generates one file with const arrays and derived union types. The Zod schemas in `schemas.ts` then derive from the generated const arrays, eliminating all duplication.
...
export const SKILL_IDS = [
  ...
  // ... all ~86 values, sorted alphabetically
] as const;
...
export { SKILL_IDS, SKILL_SLUGS } from "./generated/source-types";
```

- Keep `PluginSkillRef` as `` `${SkillId}:${SkillId}` ``. With exact unions this becomes an 85×85 cross-product (7,225 members) — try it first. If TS/IDE performance degrades, fall back to `` `${SkillIdPrefix}-${string}-${string}:${SkillIdPrefix}-${string}-${string}` `` (structural validation without the cross-product).
...
```typescript
import { SKILL_IDS, SKILL_SLUGS, CATEGORIES, AGENT_NAMES } from "../../types";
...
Delete: `CATEGORY_VALUES` const array (replaced by `CATEGORIES` from generated file). Keep `isValidSkillId()` regex for runtime validation at parse boundaries.
...
3. Update `types/skills.ts` — re-export `SkillId`, `SkillSlug`, `SKILL_IDS`, `SKILL_SLUGS`
