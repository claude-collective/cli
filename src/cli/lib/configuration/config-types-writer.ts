...
export function generateConfigTypesSource(
  ...
  const extraCategoriesArr = (extras?.extraCategories ?? []) as Subcategory[];
  ...
  const subcategories = unique([...typedKeys(matrix.categories), ...extraCategoriesArr]).sort();
  ...
  const customCategorySet = new Set<Subcategory>(extraCategoriesArr);
  for (const id of typedKeys(matrix.skills)) {
    const skill = matrix.skills[id];
  ...
  const subcategoryLine = formatMaybeSectionedUnion(subcategories, (s) => customCategorySet.has(s));
