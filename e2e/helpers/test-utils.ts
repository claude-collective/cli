 ...
 * a config that references it via a custom subcategory in the stack.
 *
 * This exercises the regression where Zod schema validation in
 * loadProjectConfigFromDir rejected custom skill IDs and subcategory keys.
 *
...
export async function createProjectWithCustomSkill(tempDir: string): Promise<{
  ...
  // Auto-generated config-types.ts with custom skill ID and custom subcategory
...
`;
  ...
  // Config file that references custom skill and custom subcategory
