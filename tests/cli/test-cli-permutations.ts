...
async function testSuggestedStacks(matrix: MergedSkillsMatrix) {
  ...
  for (const stack of matrix.suggestedStacks) {
    ...
    if (missingSkills.length > 0) {
      stats.failed++;
      logError(
        `Stack "${stack.name}" (${stack.id}): Missing skills: ${missingSkills.join(", ")}`,
      );
      results.push({
        path: `stack/${stack.id}`,
        success: false,
    ...
    if (!validation.valid) {
      ...
      logError(
        `Stack "${stack.name}" (${stack.id}): Validation errors: ${errors}`,
      );
      results.push({
        path: `stack/${stack.id}`,
        success: false,
    ...
    if (validation.warnings.length > 0) {
      ...
      logWarning(
        `Stack "${stack.name}" (${stack.id}): Warnings: ${warnings.join("; ")}`,
      );
      results.push({
        path: `stack/${stack.id}`,
        success: true,
    ...
    stats.passed++;
    logSuccess(
      `Stack "${stack.name}" (${stack.id}): Valid with ${stack.allSkillIds.length} skills`,
    );
  }
}
...
async function testCategoryNavigation(matrix: MergedSkillsMatrix) {
  ...
  log(
    `Found ${topCategories.length} top-level categories: ${topCategories.join(", ")}`,
  );
  ...
  for (const topCatId of topCategories) {
    ...
    for (const subCatId of subcategories) {
      ...
      if (skills.length === 0) {
        ...
        logSuccess(
          `  ${subCat.name} (${subCatId}): ${skills.length} skills - ${skillNames}`,
        );
        results.push({
          path: `category/${topCatId}/${subCatId}`,
          success: true,
...
async function testSkillRequirements(matrix: MergedSkillsMatrix) {
  ...
  const skillsWithRequirements = Object.values(matrix.skills).filter(
  ...
  for (const skill of skillsWithRequirements) {
    ...
    if (!canBeEnabled) {
      ...
      const reqInfo = skill.requires
        ...
        .join("; ");
      logError(
        `${skill.name}: Cannot be enabled even with requirements - ${reqInfo}`,
      );
      results.push({
        path: `requirement/${skill.alias || skill.id}`,
        success: false,
    ...
    logSuccess(
      `${skill.name}: Correctly disabled without requirements (${reason})`,
    );
    results.push({
      path: `requirement/${skill.alias || skill.id}`,
      success: true,
...
async function testSkillConflicts(matrix: MergedSkillsMatrix) {
  ...
  const skillsWithConflicts = Object.values(matrix.skills).filter(
  ...
  for (const skill of skillsWithConflicts) {
    for (const conflict of skill.conflictsWith) {
      ...
      if (!conflictSkill) {
        stats.failed++;
        logError(
          `${skill.name}: Conflicts with non-existent skill ${conflict.skillId}`,
        );
        results.push({
      ...
      if (!skillBDisabled || !skillADisabled) {
        stats.failed++;
        logError(
          `${skill.name} <-> ${conflictSkill.name}: Conflict not enforced bidirectionally`,
        );
        results.push({
      ...
      stats.passed++;
      logSuccess(
        `${skill.name} <-> ${conflictSkill.name}: Conflict enforced (${conflict.reason})`,
      );
      results.push({
        path: `conflict/${skill.alias || skill.id}/${conflictSkill.alias || conflictSkill.id}`,
        success: true,
...
async function testReactEcosystem(matrix: MergedSkillsMatrix) {
  ...
  const combinations = [
    ...
    {
      name: "React + Next.js + next-intl",
      skills: ["react", "nextjs-app-router", "next-intl"],
    },
    {
...
async function testVueEcosystem(matrix: MergedSkillsMatrix) {
  ...
  const combinations = [
    ...
    {
      name: "Vue + Nuxt Full",
      skills: ["vue", "nuxt", "tailwind", "pinia", "vee-validate", "vue-i18n"],
    },
  ];
...
async function testAngularEcosystem(matrix: MergedSkillsMatrix) {
  ...
  const combinations = [
    ...
    {
      name: "Angular + NgRx SignalStore",
      skills: ["angular", "ngrx-signalstore"],
    },
    {
      name: "Angular Full",
      skills: [
...
async function testSolidJSEcosystem(matrix: MergedSkillsMatrix) {
  ...
  const combinations = [
    ...
    {
      name: "SolidJS Full",
      skills: ["solidjs", "tailwind", "vitest", "playwright-e2e"],
    },
  ];
...
async function testBackendCombinations(matrix: MergedSkillsMatrix) {
  ...
  const combinations = [
    ...
    {
      name: "Hono + Drizzle + Better Auth",
      skills: ["hono", "drizzle", "better-auth"],
    },
    ...
    { name: "Fastify + Prisma", skills: ["fastify", "prisma"] },
    {
      ...
      skills: ["hono", "drizzle", "axiom-pino-sentry", "posthog"],
    },
  ];
...
async function testMobileCombinations(matrix: MergedSkillsMatrix) {
  ...
  const combinations = [
    ...
    {
      name: "React Native + React Query",
      skills: ["react-native", "react-query"],
    },
    {
      name: "React Native + Redux Toolkit",
      skills: ["react-native", "redux-toolkit"],
    },
    {
      name: "React Native Full",
      skills: [
        "react-native",
        "zustand",
        "react-query",
...
async function testInvalidCombinations(matrix: MergedSkillsMatrix) {
  ...
  const invalidCombinations = [
    ...
    {
      name: "React + Angular (framework conflict)",
      skills: ["react", "angular"],
    },
    {
      name: "Next.js + Remix (meta-framework conflict)",
      skills: ["react", "nextjs-app-router", "remix"],
    },
    {
      name: "SCSS + Tailwind (styling conflict)",
      skills: ["scss-modules", "tailwind"],
    },
    {
      name: "Zustand + Redux (state conflict)",
      skills: ["react", "zustand", "redux-toolkit"],
    },
    {
      name: "Zustand without React (missing requirement)",
      skills: ["zustand"],
    },
    { name: "Pinia without Vue (missing requirement)", skills: ["pinia"] },
    {
      name: "NgRx without Angular (missing requirement)",
      skills: ["ngrx-signalstore"],
    },
    {
      name: "Better Auth without Drizzle (missing requirement)",
      skills: ["hono", "better-auth"],
    },
    ...
    {
      name: "shadcn without Tailwind (missing requirement)",
      skills: ["react", "shadcn-ui"],
    },
    {
      name: "Playwright + Cypress (E2E conflict)",
      skills: ["playwright-e2e", "cypress-e2e"],
    },
    { name: "Hono + Express (API conflict)", skills: ["hono", "express"] },
    {
      name: "Drizzle + Prisma (ORM conflict)",
      skills: ["hono", "drizzle", "prisma"],
    },
  ];
...
async function testAliasMapping(matrix: MergedSkillsMatrix) {
  ...
  if (missingMappings.length === 0) {
    logSuccess(
      `All ${Object.keys(matrix.aliases).length} aliases map to existing skills`,
    );
  }
}
...
async function main() {
  ...
  try {
    ...
    log(
      `Loaded ${Object.keys(matrix.skills).length} skills, ${matrix.suggestedStacks.length} stacks`,
    );
  } catch (error) {
